// This script is used for doing bootstrapping analysis to evaluate the model uncertainties. Contributed by Johan van den Hoogen.
...
var forest_collection = ee.FeatureCollection("your directory/forest_rsr_sample_for_grsearch_20200517");
...
// comp = comp.addBands(tc);
    comp = comp.addBands(tn);
    comp = comp.addBands(cnratio);

// Make a list of covariates to use
var covarsToUse_Current = forest_collection.first().propertyNames().removeAll([
  'system:index',
	'index',
	'ratio',
	'Pixel_Long',
...
print(covarsToUse_Current);
    comp = comp.select(covarsToUse_Current.sort());

var orig_prediction = ee.Image('users/haozhima95/rootshootratio/forest_rmf_model_aggregate_first_1_20200517')
...
Export.table.toAsset({
	...
	description: 'rmf_forest_sampled_points',
	assetId: 'users/johanvandenhoogen/2021_rmf_rerun/rmf_forest_sampled_points_wBiome'
});

// var sampled_points = ee.FeatureCollection('users/johanvandenhoogen/2021_rmf_rerun/rmf_forest_sampled_points_wBiome')//.set('band_order', covarsToUse_Current)
...
seedsForBootstrapping.map(function(seedToUse) {
	...
	Export.image.toAsset({
		...
		assetId: 'projects/crowtherlab/johan/2021_rmf_rerun/forest_bootstrapColls/' + bootstrapFileName + pad(seedToUse,2),
		region: unboundedGeo,
		crs: 'EPSG:4326',
...
var bootstrapIterations = ee.ImageCollection('projects/crowtherlab/johan/2021_rmf_rerun/forest_bootstrapColls')
...
Export.image.toAsset({
	...
	description: 'rmf_forest_bootstrapped_images',
	assetId: 'users/johanvandenhoogen/2021_rmf_rerun/forest_rmf_Bootstrapped_MultibandImage',
	region: unboundedGeo,
	crs: 'EPSG:4326',
	crsTransform: [0.008333333333333333, 0, -180, 0, -0.008333333333333333, 90],
	maxPixels: 1e13
